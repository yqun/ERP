    ...
    <group :gutter="-1" v-for="(item,index) of questionList" :key="item.id">
      ...
      <cell v-for="option of item.questionOptionList" :key="option.id">
        ...
        <div class="option_val">
          <span class="val_num">{{option.baifenbi}}%({{customerCount}}人)</span>
          <span v-if="option.baifenbi > 50" class="val_chart" :style="{'width':option.baifenbi + '%','backgroundColor':'red'}">隐藏的文字，撑开span的高度</span>
          <span v-else-if="option.baifenbi > 30" class="val_chart" :style="{'width':option.baifenbi + '%','backgroundColor':'green'}">隐藏的文字，撑开span的高度</span>
          <span v-else-if="option.baifenbi >= 0" class="val_chart" :style="{'width':option.baifenbi + '%','backgroundColor':'green'}">隐藏的文字，撑开span的高度</span>
        </div>
      </cell>
      ...
      .then(res => {
        ...
        questionList.forEach(ques => {
          ques.questionOptionList.forEach(opt => {
            opt.baifenbi = (opt.selectedCount/customerCount*100).toFixed(2)*1
          })
        })
